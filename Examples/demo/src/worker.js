...
function debug(event, details = {}) {
  console.info("[Vosklet Challenge Worker]", event, details);
}
...
async function startRecording() {
  ...
  try {
    ...
    console.error("[Vosklet Challenge Worker] microphone-request-failed", error);
...
async function finishRecording(blocks) {
  ...
  try {
    ...
    console.error("[Vosklet Challenge Worker] recording-processing-failed", error);
...
async function initialize() {
  try {
    ...
    // Same engine API as createVoskletMono(), but the runtime boots inside a
    // dedicated Web Worker: recognition never blocks the UI thread, and no
    // SharedArrayBuffer, COOP, or COEP is required.
    ...
    console.error("[Vosklet Challenge Worker] initialization-failed", error);
    status.textContent = error.message;
