...
| Function/Object | Description |
|-|-|
| `Promise<Model> createModel(url: string, path: string, id: string)`<br><br>`Promise<SpkModel> createSpkModel(url: string, path: string, id: string)` | Create a `Model` or `SpkModel`. Model files must be directly under the model root. Archive format must be USTAR tar, either directly (`.tar`) or gzip-compressed tar bytes (`.tar.gz`/`.tgz`). URL extension is not used for format detection. If:<br>- `path` contains valid model files and `id` is the same, there will not be a fetch from `url`.<br>- `path` doesn't contain valid model files, or if it contains valid model files but `id` is different, there will be a fetch from `url`, and the model is stored with `id`. Models are thread-safe and reusable across recognizers. |
...
| `finalResult()` | Flush remaining buffered audio and return a final JSON result string. Use this in batch/file flows after the last `acceptWaveform()` chunk. |
| `reset()` | Reset recognizer state before reusing an existing recognizer instance |
...
Vosklet now supports two explicit build modes:

- **threaded** (default): keeps existing worker/atomics build (`-sWASM_WORKERS`, `-matomics`). Requires `SharedArrayBuffer`, COOP, and COEP.
- **singlethread**: excludes `-sWASM_WORKERS` and `-matomics`, suitable for Android WebView/Capacitor environments without cross-origin isolation.

Build commands:

```shell
cd src
# Default threaded artifact
./make

# Single-threaded fallback artifact
VOSKLET_MODE=singlethread OUTPUT_BASENAME=Vosklet.single ./make
```

Single-thread mode is usually slower and should run in a dedicated regular Web Worker when practical to avoid blocking UI.
...
- Required for **threaded** builds (`VOSKLET_MODE=threaded`).
- Not required for **singlethread** builds (`VOSKLET_MODE=singlethread`).

If you cannot set COOP/COEP for threaded mode, you may use a workaround in [AddCOI.js](AddCOI.js).

## Model archive hosting

- The wrapper accepts extensionless model URLs (for example Android `https://appassets.androidplatform.net/models/en-small`).
- Format detection is based on fetched bytes, not URL extension.
- Serve model bytes as `application/octet-stream`.
- If serving gzip-compressed tar bytes for JS-side decompression, **do not set** `Content-Encoding: gzip`.
- The wrapper throws explicit errors when gzip decompression is unavailable (`DecompressionStream` missing) or bytes are not valid USTAR tar data.
...
| VOSKLET_MODE | Build mode: `threaded` or `singlethread` | `threaded` |
| OUTPUT_BASENAME | Output filename base for generated JS/WASM | `Vosklet` |
